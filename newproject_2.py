...
window.title('Welcome to K V O F KATNI')
window.configure(background='coral')
a=ImageTk.PhotoImage(Image.open('pic 1 1.png'))
Label(window,image=a).place(x=5,y=70)
...
def start():
    ...
    mode.set("Mode") # default value
    var = OptionMenu(window,mode, "Faculty Mode", "Student Mode")
    var.grid(row=2,column=4)

    def login():
        if((((v.get())!='anupam') or ((vv.get())!='anupam')) and (mode.get()!="Faculty Mode" or mode.get()!="Student Mode")):                     #make for mode
            showinfo('Login Failed','Incorrect Id Or Password Used Or Mode Not Selected')
        else:
            menu = Toplevel()
...
Button(window,image=b,command=start,bd=-5).place(x=410,y=590)

Label(window,text='Welcome \n\n\nContact Us:\nahdev2020@gmail.com\n\n Developer:\nAnupam Haldkar\nLNCT,Bhopal(M.P)',font=("Times New",20),bg='cyan',fg='black').place(x=1090,y=220)
